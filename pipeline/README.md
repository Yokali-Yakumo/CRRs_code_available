> **Note on paths.** The layout diagrams and relative paths below are
> relative to this directory (`pipeline/`), which is the repository
> root used by every script (`ROOT` is derived from the script's own
> location). See the top-level `README.md` for the whole repository.
...
`run_03` writes the clustering solution used in the manuscript. The number of
clusters is a function of the MFA variance-explained threshold, the clustering
resolution and the graph `k.param`; `run_05` sweeps exactly those parameters.

Across that parameter grid the robust feature is the **two-neighbourhood
structure** of the UMAP embedding — predominantly repressive CRRs versus CRRs
carrying activation-associated features — which is what the downstream
biological conclusions rely on. See the top-level `README.md` for details.
